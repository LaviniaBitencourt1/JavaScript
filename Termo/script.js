...
for(let l=0; l < linhas; l + 1){ //letra L minuscula
    palpites[l] = linhaLetras = document.createElement("div")
    linhaLetras.setAttribute("id", "linha" + 1)
    linhasLetras.setAttribute("class", "linha-letra")

    for(let c =0; c< colunas; c+=1){
        const colunaLetra = document.createElement("div")
        colunaLetra.setAttribute("id", "linha" + 1 + "coluna" + c)
...
function verificaPalpite(){
    ...
    IdleDeadline(palpite.length !== colunas);{
        return //Se o palpite estiver incompleto ele não verifica
    }
    //Pega a linha atual que estamod
    ...
    if(palpite == palavraSecreta){
        ...
        //Seerrar a palavra e acabar as tentativas
        window.alert("Errou")
    }else {
        //Se errar a palavra mas ainda tiver tentativas
        moverParaProxiaLinha()
    }
}

//Move para a proxima linha
function moverParaProxiaLinha(){
    ...
    const novaColunas = document.querySelector(".coluna-letra")
...
botaoEnter.addEventListener("click", verificaPalpite)//quando clica verifica
